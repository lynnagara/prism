//! What OpenTelemetry puts on the wire, as rows.
...
/// one row per span.
pub fn from_request(request: ExportTraceServiceRequest, received_at: DateTime<Utc>) -> Vec<Span> {
    request
        .resource_spans
        .into_iter()
        .flat_map(|resource| {
...
mod tests {
    ...
    fn a_span_becomes_a_record() {
        let spans = from_request(request(vec![finished()]), at(11));
        let span = &spans[0];
    ...
    fn a_span_still_running_has_no_end() {
        ...
        let spans = from_request(request(vec![running]), at(11));
    ...
    fn every_span_gets_its_resources_attributes() {
        ...
        for span in from_request(request, at(11)) {
    ...
    fn a_span_attribute_wins_over_the_resources() {
        ...
        let spans = from_request(request, at(11));
    ...
    fn every_span_in_the_request_is_returned() {
        ...
        assert_eq!(from_request(request, at(11)).len(), 4);
    }
}
